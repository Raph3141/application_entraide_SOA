...
public class DemandeResource {
    ...
    public ResponseEntity<?> deleteDemande(@PathVariable int id) {
        try {
            boolean deleted = repo.deleteDemande(id);

            if (!deleted) {
                return ResponseEntity
                        .status(HttpStatus.NOT_FOUND)
                        .body(Map.of("erintror", "Demande not found or could not be deleted"));
            }
    ...
    public ResponseEntity<?> getDemandeById(@PathVariable Integer id) {
        try {
            Demande demande = repo.getDemandeById(id);
    ...
    public ResponseEntity<?> getAllDemandes() {
        try {
            List<Demande> demandes = repo.getAllDemandes();
